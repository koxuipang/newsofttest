...
*>修改人:
*>日期:
*>描述: 实现ITEAD-PLUS对SIM900模块的支持
*>备注: 修改底层函数，功能函数未修改，短信正常
********************************************************************************/
...
#define uart_device	UART4

SoftwareSerial mySerial;  //rx, tx

#ifdef DEBUG_LED_ENABLED
  int DEBUG_LED = 63;                // LED connected to digital pin 
...
void GSM::RxInit(uint16_t start_comm_tmout, uint16_t max_interchar_tmout)
{
  ...
  comm_buf_len = 0;
  //mySerial.flush(uart_device); // erase rx circular buffer
}
...
byte GSM::IsRxFinished(void)
{
  ...
  if (rx_state == RX_NOT_STARTED) {
    ...
    if (!mySerial.available(uart_device)) {
  ...
  if (rx_state == RX_ALREADY_STARTED) {
    ...
    num_of_bytes = mySerial.available(uart_device);
    ...
    while (num_of_bytes) {
      ...
      if (comm_buf_len < COMM_BUF_LEN) {
        ...
        // to the rx buffer
        *p_comm_buf = mySerial.read(uart_device);
...
byte GSM::IsStringReceived(char const *compare_string)
{
  ...
  /*
	...
	*/
	...
	printf("NOT received string is %s\n",compare_string);
	  #endif
	}
  }
                ...
                byte no_of_attempts)
{
  ...
  for (i = 0; i < no_of_attempts; i++) {
    ...
    mySerial.println(uart_device,AT_cmd_string);
...
void GSM::TurnOn(long baud_rate)
{
  ...
  mySerial.begin(uart_device,baud_rate);
	...
	if (AT_RESP_ERR_DIF_RESP == SendATCmdWaitResp("AT", 500, 100, "OK", 5)) {		//check OK
			...
			DebugPrint("DEBUG: the baud is not ok\r\n", 0);
			  ...
			  for (int i=1;i<7;i++){
					switch (i) {
					case 1:
					  mySerial.begin(uart_device,4800);
						#ifdef DEBUG_PRINT
							DebugPrint("DEBUG: provo Baud 4800\r\n", 0);
						#endif
					  break;
					case 2:
					  mySerial.begin(uart_device,9600);
					  #ifdef DEBUG_PRINT
							DebugPrint("DEBUG: provo Baud 9600\r\n", 0);
						#endif
					  break;
					case 3:
					  mySerial.begin(uart_device,19200);
					  #ifdef DEBUG_PRINT
							DebugPrint("DEBUG: provo Baud 19200\r\n", 0);
						#endif
					  break;
					case 4:
					  mySerial.begin(uart_device,38400);
					  #ifdef DEBUG_PRINT
							DebugPrint("DEBUG: provo Baud 38400\r\n", 0);
						#endif
					  break;
					case 5:
					  mySerial.begin(uart_device,57600);
					  #ifdef DEBUG_PRINT
							DebugPrint("DEBUG: provo Baud 57600\r\n", 0);
						#endif
					  break;
					case 6:
					  mySerial.begin(uart_device,115200);
					  #ifdef DEBUG_PRINT
				  ...
				  /*sprintf (buff,"AT+IPR=%f",baud_rate);
					...
					#endif
					*/
				  mySerial.write(uart_device,"AT+IPR=");
				  mySerial.write(uart_device,baud_rate);    
				  mySerial.write(uart_device,"\r"); // send <CR>
				  delay(500);
				  mySerial.begin(uart_device,baud_rate);
				  delay(100);
...
byte GSM::CheckRegistration(void)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  mySerial.println(uart_device,"AT+CREG?");
...
byte GSM::CallStatus(void)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  mySerial.println(uart_device,"AT+CPAS");
                             ...
                             byte first_authorized_pos, byte last_authorized_pos)
{
  ...
  if (CLS_FREE != GetCommLineStatus()) return (CALL_COMM_LINE_BUSY);
  SetCommLineStatus(CLS_ATCMD);
  mySerial.println(uart_device,"AT+CLCC");
...
void GSM::PickUp(void)
{
  if (CLS_FREE != GetCommLineStatus()) return;
  SetCommLineStatus(CLS_ATCMD);
  mySerial.println(uart_device,"ATA");
  SetCommLineStatus(CLS_FREE);
}
...
void GSM::HangUp(void)
{
  if (CLS_FREE != GetCommLineStatus()) return;
  SetCommLineStatus(CLS_ATCMD);
  mySerial.println(uart_device,"ATH");
  SetCommLineStatus(CLS_FREE);
}
...
**********************************************************/
void GSM::Call(const char *number_string)
{
  if (CLS_FREE != GetCommLineStatus()) return;
  SetCommLineStatus(CLS_ATCMD);
  // ATDxxxxxx;<CR>
  mySerial.write(uart_device,"ATD");
  mySerial.write(uart_device,number_string);    
  mySerial.write(uart_device,";\r");
...
void GSM::Call(int sim_position)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  // ATD>"SM" 1;<CR>
  mySerial.write(uart_device,"ATD>\"SM\" ");
  mySerial.write(uart_device,sim_position);    
  mySerial.write(uart_device,";\r");
...
char GSM::SetSpeakerVolume(byte speaker_volume)
{
  ...
  // select speaker volume (0 to 14)
  // AT+CLVL=X<CR>   X<0..14>
  mySerial.write(uart_device,"AT+CLVL=");
  mySerial.write(uart_device,(int)speaker_volume);    
  mySerial.write(uart_device,"\r"); // send <CR>
  // 10 sec. for initial comm tmout
  // 50 msec. for inter character timeout
...
char GSM::SendDTMFSignal(byte dtmf_tone)
{
  ...
  SetCommLineStatus(CLS_ATCMD);
  // e.g. AT+VTS=5<CR>
  mySerial.write(uart_device,"AT+VTS=");
  mySerial.write(uart_device,(int)dtmf_tone);    
  mySerial.write(uart_device,"\r");
...
**********************************************************/
char GSM::SendSMS(const char *number_str, const char *message_str) 
{
  char ret_val = -1;
  byte i;
  ...
  for (i = 0; i < 3; i++) {
    ...
    mySerial.write(uart_device,"AT+CMGS=\"");
    mySerial.write(uart_device,number_str);  
    mySerial.write(uart_device,"\"\r");
...
char GSM::IsSMSPresent(byte required_status) 
{
  ...
  switch (required_status) {
    case SMS_UNREAD:
      mySerial.write(uart_device,"AT+CMGL=\"REC UNREAD\"\r");
      break;
    case SMS_READ:
      mySerial.write(uart_device,"AT+CMGL=\"REC READ\"\r");
      break;
    case SMS_ALL:
      mySerial.write(uart_device,"AT+CMGL=\"ALL\"\r");
      break;
  }
...
char GSM::GetSMS(byte position, char *phone_number, char *SMS_text, byte max_SMS_len) 
{
  ...
  mySerial.write(uart_device,"AT+CMGR=");
  char posArray[4];
  //itoa(position, posArray, 10);
  sprintf(posArray,"%d",position);
//printf("%s\n",posArray); 
  mySerial.write(uart_device,posArray); // was : mySerial.write((int)position)
  mySerial.write(uart_device,"\r");
 
  // 5000 msec. for initial comm tmout
  // 100 msec. for inter character tmout
  switch (WaitResp(10000, 300, "+CMGR")) {
    case RX_TMOUT_ERR:
      // response was not received in specific time
      ret_val = -2;
      break;
...
char GSM::DeleteSMS(byte position) 
{
  ...
  mySerial.write(uart_device,"AT+CMGD=");
  mySerial.write(uart_device,(int)position);  
  mySerial.write(uart_device,"\r");
...
char GSM::GetPhoneNumber(byte position, char *phone_number)
{
  ...
  mySerial.write(uart_device,"AT+CPBR=");
  mySerial.write(uart_device,(int)position);  
  mySerial.write(uart_device,"\r");
...
char GSM::WritePhoneNumber(byte position, char *phone_number)
{
  ...
  mySerial.write(uart_device,"AT+CPBW=");
  mySerial.write(uart_device,(int)position);  
  mySerial.write(uart_device,",\"");
  mySerial.write(uart_device,phone_number);
  mySerial.write(uart_device,"\"\r");
...
void GSM::Echo(byte state)
{
	if (state == 0 or state == 1)
	{
	  ...
	  #endif
	  mySerial.write(uart_device,"ATE");
	  mySerial.write(uart_device,(int)state);    
	  mySerial.write(uart_device,"\r");
	  delay(500);
	  SetCommLineStatus(CLS_FREE);
	}
...
extern "C"{
	...
	byte CallStatusWithAuth(char *phone_number,byte first_authorized_pos, byte last_authorized_pos){
		sim.CallStatusWithAuth(phone_number,first_authorized_pos,last_authorized_pos);
	}
	void PickUp(void){
		sim.PickUp();
	}
	...
	void Call(const char *number_string){
		sim.Call(number_string);
	}
	void CallPosition(int sim_position){
		//call
		sim.Call(sim_position);
	}
	char SetSpeakerVolume(byte speaker_volume){
		sim.SetSpeakerVolume(speaker_volume);
	}
	char IncSpeakerVolume(void){
		sim.IncSpeakerVolume();
	}
	char DecSpeakerVolume(void){
		sim.DecSpeakerVolume();
	}
	char SendDTMFSignal(byte dtmf_tone){
		sim.SendDTMFSignal(dtmf_tone);
	}
	byte IsUserButtonEnable(void) {
		sim.IsUserButtonPushed();
	}
	void DisableUserButton(void) {
		sim.DisableUserButton();
	}
	void EnableUserButton(void) {
		sim.EnableUserButton();
	}
	byte IsUserButtonPushed(void){
		sim.IsUserButtonPushed();
	}  
	char SendSMS(const char *number_str, const char *message_str){
		sim.SendSMS(number_str,message_str);
	}
	char SendSMSPosition(byte sim_phonebook_position, char *message_str){
		//SendSMS
		sim.SendSMS(sim_phonebook_position,message_str);
	}
	char IsSMSPresent(byte required_status){
		sim.IsSMSPresent(required_status);
	}
	char GetSMS(byte position, char *phone_number, char *SMS_text, byte max_SMS_len){
		sim.GetSMS(position,phone_number,SMS_text,max_SMS_len);
	}
	char GetAuthorizedSMS(byte position, char *phone_number, char *SMS_text, byte max_SMS_len,byte first_authorized_pos, byte last_authorized_pos){
		sim.GetAuthorizedSMS(position,phone_number,SMS_text,max_SMS_len,first_authorized_pos,last_authorized_pos);
	}
	char DeleteSMS(byte position){
		sim.DeleteSMS(position);
	}
	char GetPhoneNumber(byte position, char *phone_number){
		sim.GetPhoneNumber(position,phone_number);
	}
	char WritePhoneNumber(byte position, char *phone_number){
		sim.WritePhoneNumber(position,phone_number);
	}
	char DelPhoneNumber(byte position){
		sim.DelPhoneNumber(position);
	}
	char ComparePhoneNumber(byte position, char *phone_number){
		sim.ComparePhoneNumber(position,phone_number);
	}
	void RxInit(uint16_t start_comm_tmout, uint16_t max_interchar_tmout){
		sim.RxInit(start_comm_tmout,max_interchar_tmout);
	}
	byte IsRxFinished(void){
		sim.IsRxFinished();
	}
	byte IsStringReceived(char const *compare_string){
		sim.IsStringReceived(compare_string);
	}
	byte WaitResp(uint16_t start_comm_tmout, uint16_t max_interchar_tmout){
		sim.WaitResp(start_comm_tmout,max_interchar_tmout);
	}
	byte WaitRespString(uint16_t start_comm_tmout, uint16_t max_interchar_tmout,char const * expected_resp_string){
		//WaitResp
		sim.WaitResp(start_comm_tmout,max_interchar_tmout,expected_resp_string);
	}
	char SendATCmdWaitResp(char const *AT_cmd_string, uint16_t start_comm_tmout, uint16_t max_interchar_tmout,char const * response_string,byte no_of_attempts){
		sim.SendATCmdWaitResp(AT_cmd_string,start_comm_tmout,max_interchar_tmout,response_string, no_of_attempts);
	}			 
